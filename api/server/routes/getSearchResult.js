...
router.get('/:id', async (req, res) => {
  const { id } = req.params;
  logger.debug(`[getSearchResult] Received request for id: ${id}`);
  ...
  try {
    ...
    res.json({
      acordao_id: doc._id,
      sumario_ia: doc.sumario_ia || null,
    });
  } catch (error) {
    logger.error(`[getSearchResult] Error fetching from Justina DB for acordao_id: ${id}`, error);
